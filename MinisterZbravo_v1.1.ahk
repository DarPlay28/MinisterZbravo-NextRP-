;Перед использованием бинда требуется изменить данные теги на свои данные: #Звание, #Имя, #Фамилия, #Город, #(1-НСК, 2-ГРК), #Патруль-, #Состав:, #Оперблок №, #Дата принятия(дд.мм.гггг), #Машина:
...
!NumPad4:: ; Взять документы на проверку
...
!Numpad5:: ; Вернуть документы владельцу
	SendMessage, 0x50,, 0x4190419,, A
...
!l:: ; Справка о нетрудоспособности
	SendMessage, 0x50,, 0x4090409,, A
...
!Numpad1:: ; Проверить на алкогольное опьянение
	SendMessage, 0x50,, 0x4190419,, A
...
!Numpad7:: ; Сеанс для наркозависимых
	SendMessage, 0x50,, 0x4190419,, A
...
!y:: ; Удостоверение
	...
	bind("#Звание г.#Город #Имя #Фамилия.", 0, 300)
	...
	bind("/do Удостоверение №#Номер удостоверения.", 0, 300)
	bind("/do Удостоверение действительно до #Дата принятия(дд.мм.гггг)г.", 0, 300)
Return

!F6:: ; Доклад об учениях (Выезд на задание)
	SendMessage, 0x50,, 0x4190419,, A
	pbind()
	bind("ОС | Докладываю. Выезжаю на учение.", 4, 300)
Return

!F7:: ; Оперблок №1 (Выезд на задание)
	SendMessage, 0x50,, 0x4190419,, A
	pbind()
	bind("ОС | Докладываю. Приступаю к выполнению операции.#Оперблок №1.Состояние стабильное.", 4, 300)
Return

!F8:: ; Патруль (Выезд на задание)
	SendMessage, 0x50,, 0x4190419,, A
	pbind()
